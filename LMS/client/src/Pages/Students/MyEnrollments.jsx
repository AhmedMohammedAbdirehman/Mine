      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-10'>
        <thead className='text-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden'>
          <tr>
            ...
            <th className='px-4 py-3 font-semibold truncate'>Durtion</th>
        ...
        <tr key={{index}} className='border-b border-gray-500/20'>
          ...
          <td className='px-4 py-3 max-sm-hidden'>
           ...
           <td className='x-4 py-3 max-sm-hidden'> 
           {progressArray[index] && `${progressArray[index].lecturesCompleted} / ${progressArray[index].totalLectures}`} <span>Lectures</span>
           </td>
           <td className='x-4 py-3 max-sm:text-right'>
            <button onClick={()=>navigate('/player/'+course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:tetx-xs text-white'>
